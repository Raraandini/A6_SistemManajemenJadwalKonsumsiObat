    ...
    public partial class FormObat : Form
    {
        ...
        private void LoadData()
        {
            try
            {
                using (SqlConnection conn = DatabaseHelper.GetConnection())
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("SELECT * FROM Obat WHERE id_user=@id_user", conn))
                    {
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = DatabaseHelper.GetConnection())
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand(
                        "UPDATE Obat SET nama_obat=@nama_obat, jumlah_stok=@jumlah_stok WHERE id_obat=@id_obat AND id_user=@id_user",
                        conn))
                    {
                        cmd.Parameters.AddWithValue("@nama_obat",   txtNamaObat.Text.Trim());
                        cmd.Parameters.AddWithValue("@jumlah_stok", stok);
                        ...
                        {
                            MessageBox.Show("Gagal mengupdate. Data tidak ditemukan atau Anda tidak memiliki akses.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                }
            }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("DELETE FROM Obat WHERE id_obat=@id_obat AND id_user=@id_user", conn))
                    {
                        cmd.Parameters.AddWithValue("@id_obat", selectedIdObat);
                        cmd.Parameters.AddWithValue("@id_user", Session.IdUser);
                        int rowsAffected = cmd.ExecuteNonQuery();
                        ...
                        {
                            MessageBox.Show("Gagal menghapus. Data tidak ditemukan atau Anda tidak memiliki akses.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                }
            }
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = DatabaseHelper.GetConnection())
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(
                        "SELECT * FROM Obat WHERE nama_obat LIKE @keyword AND id_user=@id_user", conn))
                    {
